...
class Snake(QtGui.QWidget):
	...
	def scoreBoard(self, qp):
		...
		qp.setBrush(QtGui.QColor(0,255,255,255))
		qp.drawRect(0, 0, 500, 24)

	def scoreText(self, event, qp):
		qp.setPen(QtGui.QColor(25,80,0,255))
	...
	def gameOver(self, event, qp):
		...
		qp.drawText(180,200, "press space to play again")

	def checkStatus(self, x, y):
		if y > 456 or x > 456 or x < 0 or y < 24:
			self.pause()
			self.isPaused = True
			self.isOver = True
			return False
	...
	def placeFood(self, qp):
		if self.FoodPlaced == False:
			self.foodx = randrange(19)*12
			self.foody = randrange(2, 19)*12
